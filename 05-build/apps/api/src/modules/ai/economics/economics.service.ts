...
export class EconomicsService {
  ...
  async report(user: RequestUser) {
    ...
    const { rows, prices } = await this.prisma.withTenant(user.tenantId, async (tx) => ({
      rows: await tx.aiInteraction.findMany({
        where: { at: { gte: since } },
        select: {
          agent: true, model: true, status: true,
    ...
    return {
      ...
      estimated: true,
      basis: 'token = heuristic mock (~4 ký tự/token, gồm context) · giá = niêm yết Anthropic (as-of theo ai_model_price) · KHÔNG phải hóa đơn thật',
